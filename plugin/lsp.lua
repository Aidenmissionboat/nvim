...
local function on_attach(client)
  ...
  if client.server_capabilities.documentFormattingProvider then
    buf_keymap(0, 'n', '<leader>f', '<cmd>lua vim.lsp.buf.format()<cr>', keymap_opts)
    buf_keymap(0, 'v', '<leader>f', '<cmd>lua vim.lsp.buf.range_formatting()<cr>', keymap_opts)
  end
...
local servers = {
  ...
  texlab = {
    settings = {
      texlab = {
        build = {
            args = { "-lualatex", "-shell-escape", "-file-line-error", "-synctex=1", "-interaction=nonstopmode", "%f" },
            onSave = true,
